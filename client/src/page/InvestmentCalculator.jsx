    ...
    <section className="min-h-screen flex flex-col items-center justify-center bg-gray-900 text-white p-6">
      <div className="max-w-6xl mx-auto bg-gray-800 p-10 rounded-3xl shadow-xl border border-gray-700">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 text-lg">Calculate your potential earnings with our staking simulator</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Investment Input Section */}
          <div className="p-6 bg-gray-700 rounded-2xl border border-gray-600 shadow-md">
            <h2 className="text-2xl font-bold text-white mb-6">Investment Parameters</h2>
            ...
            <p className="text-purple-400 text-sm mt-2 text-center font-semibold">
              {amount} SOL <span className="text-gray-500 text-xs">(~${(amount * SOL_PRICE).toFixed(2)} USD)</span>
            </p>

            {/* Duration Slider */}
            <label className="text-gray-400 mt-4">Staking Duration (Days)</label>
            <input 
              type="range" 
              min="7" 
              max="90" 
            ...
            />
            <p className="text-blue-400 text-sm mt-2 text-center font-semibold">{duration} Days</p>
          </div>

          {/* Staking Returns Section */}
          <div className="p-6 bg-gray-700 rounded-2xl border border-gray-600 shadow-md">
            <h2 className="text-2xl font-bold text-white mb-6">Staking Returns</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-purple-600/10 p-6 border border-purple-500/20 rounded-xl text-center">
                ...
                <h3 className="text-2xl font-bold text-purple-400">{dailyProfit.toFixed(4)} SOL</h3>
                <p className="text-gray-500 text-xs">(~${(dailyProfit * SOL_PRICE).toFixed(2)} USD)</p>
              </div>
              <div className="bg-blue-600/10 p-6 border border-blue-500/20 rounded-xl text-center">
                <p className="text-gray-300">Total ROI</p>
                <h3 className="text-2xl font-bold text-blue-400">{(roi * 100).toFixed(2)}%</h3>
              </div>
              <div className="bg-green-600/10 p-6 border border-green-500/20 rounded-xl text-center">
                ...
                <h3 className="text-2xl font-bold text-green-400">{totalReturn.toFixed(2)} SOL</h3>
                <p className="text-gray-500 text-xs">(~${(totalReturn * SOL_PRICE).toFixed(2)} USD)</p>
              </div>
              <div className="bg-gray-600/10 p-6 border border-gray-500/20 rounded-xl text-center">
                <p className="text-gray-300">Net Profit</p>
                <h3 className="text-2xl font-bold text-gray-300">{netProfit.toFixed(2)} SOL</h3>
                <p className="text-gray-500 text-xs">(~${(netProfit * SOL_PRICE).toFixed(2)} USD)</p>
              </div>
            </div>
          </div>
        </div>
